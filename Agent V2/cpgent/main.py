...
async def scrape_leetcode(url: str) -> ScraperOutput:
    ...
    # ADVANCED FEATURE: Parse daily submission calendar for Unified Chronology
    daily_activity = {}
...
async def scrape_codeforces(url: str) -> ScraperOutput:
    ...
    # ADVANCED FEATURE: Fetch daily activity for Unified Chronology
    daily_activity = {}
...
# ─── Advanced Scoring Math Engines ────────────────────────────────────────────

def clamp(v: float, lo: float = 0.0, hi: float = 100.0) -> float:
    return max(lo, min(hi, v))

def apply_persona_weights(clout: float, cons: float, vel: float) -> Tuple[float, str]:
    """Applies the correct persona logic based on the user's sub-scores."""
...
def extract_raw_subscores_leetcode(profile: ScraperOutput, config: dict) -> dict:
    ...
    # 30 active days in a quarter = 100% consistency (Roughly 1/3 of days)
...
async def async_execute(platforms: dict, config: dict) -> dict:
    ...
    # 1. Process Raw Sub-Scores & Apply Dynamic Personas
    scored_platforms = {}
    unified_daily = {}
    ...
    for idx, res in enumerate(results):
        ...
        if isinstance(res, Exception):
            print(f"[-] CP Agent Warning: Scraper failed for {plat_key} - {str(res)}")
            continue
        
        # Merge unified daily chronology
    ...
    if not scored_platforms:
        return {"status": "failed", "error_log": "All scrapers failed or timed out", "final_score": 0}

    # 2. Calculate Platform Confidence Index (Dynamic Self-Normalizing Weight)
    # CP_p = ln(Total Submissions + 1) * (S_clout / 100)
    cp_indices = {}
    for p_name, p_data in scored_platforms.items():
        cp_val = math.log(p_data["solved_count"] + 1) * (p_data["clout"] / 100)
        cp_indices[p_name] = max(cp_val, 0.01) # Baseline 0.01 to prevent div zero
    ...
    # 3. Apply Unified Activity Chronology & Sigmoidal Scaling Engine (M_global)
    # Generate 365-day layout
    ...
    # [Single-Weekend Crunch-Risk Detection Upgrade]
    total_year_volume = sum(timeline)
    max_3day_burst = max([sum(timeline[i:i+3]) for i in range(len(timeline)-2)]) if len(timeline) >= 3 else total_year_volume
    crunch_ratio = (max_3day_burst / max(1, total_year_volume)) if total_year_volume > 0 else 0.0
    single_weekend_crunch_risk = crunch_ratio > 0.45 and total_year_volume >= 30

    mu_global = total_year_volume / 365
    ...
    # Apply crunch risk penalty if flagged
    if single_weekend_crunch_risk:
        m_global *= 0.85  # 15% crunch-risk penalty for single-weekend cramming

    # [Unified Elo Difficulty Rating Scale Upgrade]
    elo_ratings = []
    for p_name, p_data in scored_platforms.items():
        count = p_data.get("solved_count", 0)
        clout = p_data.get("clout", 0.0)
        if p_name == "leetcode":
            elo = 800 + (clout * 12.0)  # Maps 0-100 clout to 800-2000 Elo
        elif p_name == "codeforces":
            elo = 800 + (clout * 17.0)  # Maps to 800-2500 Elo
        elif p_name == "codechef":
            elo = 900 + (clout * 11.0)
        else:
            elo = 800 + (clout * 8.0)
        elo_ratings.append(round(elo, 1))

    unified_elo = round(sum(elo_ratings) / max(1, len(elo_ratings)), 1)
    ...
    return {
        ...
        "narrative_context": {
            ...
            "unified_elo_rating": unified_elo,
            "single_weekend_crunch_risk": single_weekend_crunch_risk,
            "crunch_ratio_3day": round(crunch_ratio, 3)
        }
    }
...
if __name__ == "__main__":
    target_platforms = {
        "leetcode": "https://leetcode.com/u/cpcs/",
        "codeforces": "https://codeforces.com/profile/tourist",
        "hackerrank": "https://www.hackerrank.com/profile/anmol_sharma21"
    }
    
    agent_config = {"timeout": 30}
    print("Starting CP Agent execution with Advanced Math Engines...")
    agent_result = execute_cp_agent(platforms=target_platforms, config=agent_config)
    print("\n=== Execution Result ===")
