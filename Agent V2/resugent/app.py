                ...
                try:
                    ...
                    if success and os.path.exists(dest_path):
                        ...
                        if not raw_text:
                            ...
                            resume_data.total_page_count = actual_page_count
...
# ═══════════════════════════════════════════════
    ...
    with st.expander("🏗️ S_hygiene — Structural Hygiene", expanded=True):
        st.markdown(f'<div class="formula-box">S_hygiene = max(0, 100 − {C["hygiene_page_pen"]}·max(0,P−1) − {C["hygiene_link_pen"]}·L_missing − {C["hygiene_sec_pen"]}·|X_missing|)</div>', unsafe_allow_html=True)
        h = steps.get("hygiene", {})
        ...
        step(3, f"Missing sections X_missing = <span class='step-val'>{h.get('X_missing','?')}</span> → penalty = <span class='step-val'>{C['hygiene_sec_pen'] * h.get('X_missing',0)}</span>")
        step(4, f"<b>S_hygiene = max(0, 100 − {C['hygiene_page_pen']*max(0,h.get('P',1)-1)} − {C['hygiene_link_pen']*h.get('L_missing',0)} − {C['hygiene_sec_pen']*h.get('X_missing',0)}) = <span class='step-val'>{h.get('score','?')}</span></b>")
