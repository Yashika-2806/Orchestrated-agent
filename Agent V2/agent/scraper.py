...
def extract_username(url):
    ...
    # Accept bare usernames, github.com/<user>, and full https://github.com/<user> URLs.
    parsed = urlparse(raw_input if "://" in raw_input else f"https://{raw_input}")
    
    # Strict validation: If they provided a full URL, it MUST be a GitHub URL
    if "://" in raw_input and "github.com" not in parsed.netloc.lower():
        return None # Reject LeetCode, LinkedIn, etc. pasted into the GitHub column
        
    parts = [p for p in parsed.path.split('/') if p]
    if parts:
        return parts[0]

    # Fallback for plain usernames without any path segment.
    return parsed.netloc if parsed.netloc and parsed.netloc.lower() != "github.com" else raw_input
...
def run_scraper(target_input: str) -> dict:
    ...
    # Sort repos_raw manually by 'pushed_at' or 'updated_at' to ensure we deep-scrape the most active ones first
    repos_raw = sorted(repos_raw, key=lambda r: r.get('pushed_at') or r.get('updated_at') or '', reverse=True)
    ...
    for idx, repo in enumerate(repos_raw):
        ...
        if is_fork:
            # Only deep-scrape forks if they are in the top 15 most recently pushed
            if idx < 15:
            ...
            # Only deep-scrape API-heavy data for the top 15 most recently pushed repos
            if idx < 15:
                ...
                # For older repos, just grab the primary language from the repo object
                primary_lang = repo.get("language")
                if primary_lang:
                    final_output["profile"]["technologies"][primary_lang] = final_output["profile"]["technologies"].get(primary_lang, 0) + 1
